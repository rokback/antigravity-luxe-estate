    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6 md:p-8">
      ...
      <div className="relative z-10 w-full max-w-2xl bg-white rounded-2xl shadow-2xl overflow-hidden flex flex-col max-h-[90vh] animate-in fade-in zoom-in duration-200">
        ...
        <header className="px-8 py-6 border-b border-gray-100 flex justify-between items-center bg-white sticky top-0 z-30">
          <h1 className="text-2xl font-semibold tracking-tight text-gray-900">{t('filter.title')}</h1>
          <button 
            onClick={onClose}
        ...
        <div className="flex-1 overflow-y-auto p-8 space-y-10 custom-scrollbar">
          {/* Section 1: Location */}
          <section>
            <label className="block text-xs font-semibold text-gray-500 uppercase tracking-wider mb-3">{t('filter.location')}</label>
          ...
          <section>
            <div className="flex justify-between items-end mb-4">
              <label className="block text-xs font-semibold text-gray-500 uppercase tracking-wider">{t('filter.price_range')}</label>
              {(filters.minPrice || filters.maxPrice) && (
                <span className="text-sm font-medium text-mosque">
                  ${(filters.minPrice || 0).toLocaleString()} – {filters.maxPrice ? '$' + filters.maxPrice.toLocaleString() : t('filter.any')}
                </span>
              )}
            </div>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="bg-gray-50 p-3 rounded-xl border border-transparent focus-within:border-mosque/30 transition-colors">
                <label className="block text-[10px] text-gray-500 uppercase font-medium mb-1">{t('filter.min_price')}</label>
              ...
              <div className="bg-gray-50 p-3 rounded-xl border border-transparent focus-within:border-mosque/30 transition-colors">
                <label className="block text-[10px] text-gray-500 uppercase font-medium mb-1">{t('filter.max_price')}</label>
                <div className="flex items-center">
                  <span className="text-gray-400 mr-1 text-sm">$</span>
                  <input 
                    type="number"
          ...
          <section className="grid grid-cols-1 md:grid-cols-2 gap-8">
            ...
            <div className="space-y-3">
              <label className="block text-xs font-semibold text-gray-500 uppercase tracking-wider">{t('filter.property_type')}</label>
              <div className="relative">
                ...
                >
                  <option value="Any Type">{t('filter.categories.Any Type')}</option>
                  {CATEGORIES.filter(cat => cat !== 'Any Type').map(cat => (
                    <option key={cat} value={cat}>{t(`filter.categories.${cat}`) === `filter.categories.${cat}` ? cat : t(`filter.categories.${cat}`)}</option>
                  ))}
                </select>
            ...
            <div className="space-y-4">
              ...
              <div className="flex justify-between items-center">
                <span className="text-sm font-medium text-gray-900">{t('filter.bedrooms')}</span>
              ...
              {/* Baths */}
              <div className="flex justify-between items-center">
                <span className="text-sm font-medium text-gray-900">{t('filter.bathrooms')}</span>
                <div className="flex items-center space-x-3 bg-gray-50 rounded-full p-1 border border-gray-100">
                  <button 
                    type="button"
          ...
          {/* Section 4: Amenities */}
          <section>
            <label className="block text-xs font-semibold text-gray-500 uppercase tracking-wider mb-4">{t('filter.amenities')}</label>
              ...
              {AMENITIES_OPTIONS.map((amenity) => {
                ...
                return (
                  ...
                  >
                    ...
                    {t(`filter.amenity_labels.${amenity.label}`) === `filter.amenity_labels.${amenity.label}` ? amenity.label : t(`filter.amenity_labels.${amenity.label}`)}
                    {isActive && (
        ...
        <footer className="bg-white border-t border-gray-100 px-8 py-6 sticky bottom-0 z-30 flex items-center justify-between">
          ...
          >
            {t('filter.show_homes')} {totalResults} {t('filter.homes')}
