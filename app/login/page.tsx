        ...
        <div className="bg-white dark:bg-[#152e2a] rounded-2xl shadow-soft p-8 sm:p-10 border border-white/50 dark:border-mosque/20 backdrop-blur-sm">
          <div className="space-y-4">
            <button 
              ...
              className={`group w-full flex items-center justify-center gap-3 bg-white dark:bg-[#1a3833] border border-gray-100 dark:border-mosque/30 rounded-lg p-3.5 text-nordic-dark dark:text-white font-medium transition-all duration-300 hover:shadow-soft-hover hover:-translate-y-0.5 relative overflow-hidden ${loading ? 'opacity-70 cursor-not-allowed' : ''}`}
            >
              ...
              <span className="relative z-10">{loading === 'google' ? 'Connecting...' : 'Continue with Google'}</span>
            </button>
            <button 
            ...
            >
              ...
              <span className="relative z-10">{loading === 'github' ? 'Connecting...' : 'Continue with GitHub'}</span>
            </button>
          </div>
          <p className="mt-8 text-center text-sm text-nordic-dark/70 dark:text-gray-400">
            Don't have an account? <Link className="font-semibold text-mosque hover:text-mosque/80 transition-colors" href="/login">Sign up</Link>
          </p>
        </div>
